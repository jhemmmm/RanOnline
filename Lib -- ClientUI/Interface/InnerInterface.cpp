...
void CInnerInterface::ResetControl()
{
	...
	m_pPersonalLockWindow = NULL;
	m_pPersonalLockWindowDummy = NULL;
...
/*inventory,equipment,storage lock, DevArt22, 2020/02/11 */
void CInnerInterface::PersonalLockWindowResetPos()
{
	if ( !m_pPersonalLockWindow )		return;
	if ( !m_pPersonalLockWindowDummy )	return;

	m_pPersonalLockWindow->SetGlobalPos( 
		D3DXVECTOR2( m_pPersonalLockWindowDummy->GetGlobalPos ().bottom, m_pPersonalLockWindowDummy->GetGlobalPos ().right ) );
}
/*school wars, Juver, 2018/01/20 */
