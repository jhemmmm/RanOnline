...
BOOL SCHARDATA2::SETSHOPPURCHASE ( VECSHOP &vecSHOP )
{
	...
	for ( DWORD i=0; i<dwSIZE; ++i )
	{
		...
		m_mapCharged.insert(std::make_pair(std::string(sPURCHASE.strPurKey), sPURCHASE));
	}
...
BOOL SCHARDATA2::SETITEMSHOP ( VECITEMSHOP &vecSHOP )
{
	...
	for ( DWORD i=0; i<dwSIZE; ++i )
	{
		...
		m_mapItemShop.insert(std::make_pair(std::string(sITEMSHOP.strItemNum), sITEMSHOP));
	}
	return TRUE;
}
